...
<table class="data_list" cellpadding="0" cellspacing="0" width="100%">
   	...
   	<tr>
        ...
        <td rowspan="3" width="10%">
        	<s:if test="cbm.length()==4">
        		县名称
        	</s:if>
        	<s:if test="cbm.length()==6">
        		镇名称
        	</s:if>
        	<s:if test="cbm.length()==9">
        		村名称
        	</s:if>
        </td>
		<td colspan="7">资金到户方式统计(户)</td>
		<td colspan="4">项目资金到户(元)</td>
        <td rowspan="3" width="4%">备注</td>
        <td rowspan="3" width="6%">
        	<p>
        	<input type="button" value="导出" onclick="location.href='${pageContext.request.contextPath }/system/govJiangBuAction_exportJiangbuDetail.do?crid=<s:property value="crid"/>&cbm=<s:property value="cbm"/>&title=<s:property value="title"/>'" class="button43"/>
        	</p>
        	<p>&nbsp;</p>
        	<p>
