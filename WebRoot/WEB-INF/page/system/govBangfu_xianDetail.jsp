...
<table class="data_list" cellpadding="0" cellspacing="0" width="100%">
    ...
    <tr style="display:none" title="省级后方单位名称: ${ls.c1 }">
        ...
        <td>${ls.c15 }</td>
        <td>
        	<c:set var="c2len" value="${fn:length(ls.c2) }"></c:set>
        	<c:choose>
        		<c:when test="${c2len gt 4 }">
        			${fn:substring(ls.c2,0,3) }..
        		</c:when>
        		<c:otherwise>${ls.c2 }</c:otherwise>
        	</c:choose>
        </td>
        <td>
        	<c:set var="c3len" value="${fn:length(ls.c3) }"></c:set>
        	<c:choose>
        		<c:when test="${c3len gt 4 }">
        			${fn:substring(ls.c3,0,3) }..
        		</c:when>
